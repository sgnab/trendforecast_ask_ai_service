...
CHART_DETAILS_LAMBDA_NAME = os.environ.get("CHART_DETAILS_LAMBDA_NAME", "chart_details_lambda_placeholder")
BRAND_INSIGHT_LAMBDA_NAME = os.environ.get("BRAND_INSIGHT_LAMBDA_NAME", "brand_insight_placeholder")
AMAZON_RADAR_LAMBDA_NAME = os.environ.get("AMAZON_RADAR_LAMBDA_NAME", "dev_amazon_recommend_placeholder")
AWS_REGION = os.environ.get("AWS_REGION", "us-west-2")

# --- NEW: Constant for the comparison task name from Interpreter ---
# Ensure this matches the constant in interpret_query_v2.py
...
# --- Helper Functions (map_timeframe_reference, safe_title_case - unchanged) ---
def map_timeframe_reference(timeframe_ref_str: str | None) -> str:
    default_value = "12"
    logger.debug(f"Mapping timeframe '{timeframe_ref_str}'...")
    if not timeframe_ref_str: logger.debug("Null/empty -> '3'"); return "3"
    ...
    if match:
        ...
        if months in ["3", "12", "48"]:
            ...
            logger.warning(f"Invalid month count '{months}', defaulting."); return default_value
    logger.warning(f"Unmapped timeframe '{timeframe_ref_str}', defaulting.");
...
# --- Helper for parallel invocation (unchanged, but task_id usage will be important) ---
def invoke_lambda_task(lambda_name: str, payload: Dict, task_id: str, subject_name: Optional[str] = None) -> Dict:
    # subject_name here is more for chart/item specific things,
    # for category comparison, the task_id itself will be made unique
    logger.info(
        f"Starting task '{task_id}' to invoke {lambda_name} for subject: {subject_name if subject_name else 'N/A'}")
    start_time = time.time()
    response_body_outer = None
    response_body_inner = None
    # --- NEW: Add category_compared to result if applicable ---
    result = {"task_id": task_id, "data": None, "error_info": None}
    if task_id.startswith("trends_compare_"):
        result["category_compared"] = task_id.split("trends_compare_", 1)[1]
    # --- END NEW ---

    try:
        response = lambda_client.invoke(FunctionName=lambda_name, InvocationType='RequestResponse',
        ...
        if response.get('FunctionError'):
            ...
            logger.error(f"FunctionError from {lambda_name} (task '{task_id}'): {error_payload_str[:500]}...")
            try:
                error_details = json.loads(error_payload_str)
            except json.JSONDecodeError:
                error_details = {"raw_error": error_payload_str}
            error_msg = f"FunctionError in {lambda_name}: {error_details.get('errorMessage', 'Unknown')}"
            result["error_info"] = {"source": lambda_name, "error": error_msg, "details": error_details}
            if subject_name: result["error_info"]["subject"] = subject_name
        else:
            ...
            status_code = outer_payload.get("statusCode", 200)
            if status_code >= 300:
                error_msg = f"Downstream lambda {lambda_name} returned error status code: {status_code}";
                logger.error(error_msg + f" (task '{task_id}')")
                result["error_info"] = {"source": lambda_name, "error": error_msg,
                                        "details": outer_payload.get("body", outer_payload)}
                if subject_name: result["error_info"]["subject"] = subject_name
            elif isinstance(outer_payload.get("body"), str):
                response_body_inner = outer_payload["body"]
                inner_result_payload = json.loads(response_body_inner)
                result["data"] = inner_result_payload;
                logger.info(f"Successfully parsed response from {lambda_name} (task '{task_id}').")
            elif isinstance(outer_payload.get("body"), dict):
                result["data"] = outer_payload["body"];
                logger.info(f"Successfully used direct body dict from {lambda_name} (task '{task_id}').")
            elif outer_payload:  # If body is missing but outer_payload is not empty, use outer_payload as data
                result["data"] = outer_payload;
                logger.warning(
                    f"Using outer payload as data from {lambda_name} (task '{task_id}') as body was missing/invalid.")
            else:  # Body is missing and outer_payload is also empty or invalid
                error_msg = f"Response from {lambda_name} missing or invalid 'body' field, and outer payload is empty/invalid.";
                logger.error(error_msg + f" (task '{task_id}')")
                result["error_info"] = {"source": lambda_name, "error": error_msg, "details": outer_payload}
                if subject_name: result["error_info"]["subject"] = subject_name
    except ClientError as e:
        error_msg = f"Boto3 ClientError invoking {lambda_name}: {e.response['Error']['Code']}";
        logger.error(error_msg + f" (task '{task_id}')", exc_info=True)
        result["error_info"] = {"source": lambda_name, "error": error_msg};
        if subject_name: result["error_info"]["subject"] = subject_name
    except json.JSONDecodeError as e:
        error_msg = f"Failed to parse JSON response from {lambda_name}: {e}";
        raw_payload_to_log = response_body_inner if response_body_inner else (
            response_body_outer if response_body_outer else 'N/A')
        logger.error(error_msg + f" (task '{task_id}')", exc_info=True);
        result["error_info"] = {"source": lambda_name, "error": error_msg, "raw_payload": raw_payload_to_log}
        if subject_name: result["error_info"]["subject"] = subject_name
    except Exception as e:
        error_msg = f"Unexpected error during {lambda_name} processing (task '{task_id}'): {str(e)}";
        logger.exception(error_msg)
        result["error_info"] = {"source": lambda_name, "error": error_msg}
        if subject_name: result["error_info"]["subject"] = subject_name
    end_time = time.time()
    logger.info(
        f"Finished task '{task_id}' for {lambda_name} in {end_time - start_time:.3f}s. Success: {result['error_info'] is None}")
    return result
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.error(f"Could not dump incoming event: {log_e}"); logger.info(
            f"ROUTER RECEIVED EVENT type: {type(event)}")
    ...
    try:
        interpretation_result = event
        if not isinstance(interpretation_result, dict): raise ValueError(
            f"Input event is not dict: {type(interpretation_result)}")
        ...
        primary_task = interpretation_result.get(
            "primary_task")  # This will be COMPARE_CATEGORIES_TASK_NAME for comparisons
        timeframe_reference = interpretation_result.get("timeframe_reference")
        ...
        # For standard tasks, category_name_from_interpreter is the actual fashion category.
        # For COMPARE_CATEGORIES, original_context.category will be the placeholder "COMPARE_CATEGORIES".
        # The actual categories to compare are in query_subjects.comparison_subjects.
        category_name_from_interpreter_context = original_context.get("category")

        if not country_name: raise ValueError("Missing 'original_context.country'")
        if not category_name_from_interpreter_context: raise ValueError("Missing 'original_context.category'")

        specific_known_subjects = query_subjects.get("specific_known", [])  # For charts
        # --- NEW: Get comparison_subjects for comparison task ---
        comparison_subjects_list = query_subjects.get("comparison_subjects",
                                                      [])  # List of {"subject": "Name", "type": "category"}
        if not isinstance(comparison_subjects_list, list):
            logger.warning("`comparison_subjects` is not a list, defaulting to empty.")
            comparison_subjects_list = []
        # --- END NEW ---

        logger.info(
            f"Parsed Input: Task={primary_task}, Required={required_sources}, SpecificKnown={specific_known_subjects}, "
            f"ComparisonSubjects={comparison_subjects_list}, ContextCountry={country_name}, "
            f"InterpreterContextCat={category_name_from_interpreter_context}, TimeframeRef={timeframe_reference}"
        )
    except (TypeError, ValueError, AttributeError, KeyError) as e:
        try:
            logger.error(f"RAW EVENT AT PARSE FAILURE: {json.dumps(event)}")
        except:
            logger.error(f"RAW EVENT AT PARSE FAILURE type: {type(event)}")
        logger.error(f"Failed to parse input event: {e}", exc_info=True);
        raise Exception(f"Invalid input structure: {e}")

    # --- Initialize Results ---
    aggregated_results = {
        "status": "partial", "trends_data": None, "mega_trends_data": None,
        "chart_details_data": None, "brand_performance_data": None,
        "amazon_radar_data": None,
        # --- NEW: Key for comparison data ---
        "trends_data_comparison": [],  # Will be a list of dicts
        # --- END NEW ---
        "errors": []
    }
    ...
    # --- NEW: Handle COMPARE_CATEGORIES_TASK ---
    if primary_task == COMPARE_CATEGORIES_TASK_NAME and "internal_trends_category" in required_sources:
        if comparison_subjects_list:
            logger.info(f"Preparing to fetch trends for {len(comparison_subjects_list)} categories for comparison.")
            for cat_subject_obj in comparison_subjects_list:
                if isinstance(cat_subject_obj, dict) and "subject" in cat_subject_obj:
                    category_to_fetch = cat_subject_obj["subject"]
                    # Create a unique task_id for each category trend fetch in comparison
                    task_id_compare = f"trends_compare_{category_to_fetch.replace(' ', '_')}"  # Sanitize name for ID
                    payload = {"queryStringParameters": {"country": country_name, "category": category_to_fetch,
                                                         "time_frame": time_frame_trends}}
                    tasks_to_submit.append(
                        (task_id_compare, TREND_MAIN_LAMBDA_NAME, payload, None))  # subject_name not critical here
                else:
                    logger.warning(f"Skipping invalid item in comparison_subjects_list: {cat_subject_obj}")
        else:
            logger.warning("COMPARE_CATEGORIES_TASK specified, but no comparison_subjects found in query_subjects.")
    # --- END NEW ---
    else:  # Standard (non-comparison) trend fetching
        invoke_trend_main = "internal_trends_item" in required_sources or "internal_trends_category" in required_sources
        if invoke_trend_main:
            # Use category_name_from_interpreter_context for standard trend calls
            payload = {
                "queryStringParameters": {"country": country_name, "category": category_name_from_interpreter_context,
                                          "time_frame": time_frame_trends}}
            tasks_to_submit.append(("trends_single", TREND_MAIN_LAMBDA_NAME, payload, None))  # task_id "trends_single"

    # Mega Trends (uses category_name_from_interpreter_context)
    if "internal_mega" in required_sources:
        payload = {
            "queryStringParameters": {"country": country_name, "category": category_name_from_interpreter_context,
                                      "time_frame": time_frame_trends, "next_batch": "Previous", "cat_mode": "True"}}
        tasks_to_submit.append(("mega", MEGA_TRENDS_LAMBDA_NAME, payload, None))

    # Charts (uses specific_known_subjects and category_name_from_interpreter_context for the actual category)
    if bool(specific_known_subjects) and "internal_trends_item" in required_sources:  # Charts often tied to item trends
        first_subject_info = specific_known_subjects[0]
        subject_name = first_subject_info.get("subject")
        subject_type = first_subject_info.get("type")
        if subject_name and subject_type:
            time_frame_charts = map_timeframe_reference(timeframe_reference)
            if time_frame_charts not in ["12", "48"]: time_frame_charts = "48"; logger.debug(
                f"Overriding chart timeframe to '48'")

            subject_name_clean = safe_title_case(subject_name)
            # For charts, the category should be the actual fashion category, not a placeholder
            actual_category_for_charts = safe_title_case(
                category_name_from_interpreter_context if category_name_from_interpreter_context != COMPARE_CATEGORIES_TASK_NAME else subject_name_clean)  # Fallback if placeholder was somehow passed

            if subject_name_clean.lower() != actual_category_for_charts.lower():  # Compare lowercase
                category_subject_key = f"{subject_name_clean} {actual_category_for_charts}"
            else:
                category_subject_key = actual_category_for_charts

            logger.debug(
                f"Constructed category_subject for chart details: {category_subject_key} using category: {actual_category_for_charts}")
            forecast_value = "True" if primary_task == "get_forecast" else "False"
            payload = {"queryStringParameters": {"country": country_name, "category_subject": category_subject_key,
                                                 "category": actual_category_for_charts,
                                                 "time_frame": time_frame_charts, "mode": subject_type,
                                                 "forecast": forecast_value}}
            tasks_to_submit.append(("charts", CHART_DETAILS_LAMBDA_NAME, payload, subject_name))
        else:
            logger.warning(f"Skipping chart details: subject/type missing in {first_subject_info}")
            aggregated_results["errors"].append(
                {"source": CHART_DETAILS_LAMBDA_NAME, "error": "Missing subject/type for chart lookup",
                 "details": first_subject_info})

    # Brand Performance
    if "internal_brand_performance" in required_sources:
        target_brand = query_subjects.get("target_brand")
        if target_brand and isinstance(target_brand, str):
            payload = {"body": json.dumps({"competitor_domain": target_brand})}
            tasks_to_submit.append(("brand_perf", BRAND_INSIGHT_LAMBDA_NAME, payload, target_brand))
            ...
            logger.error(f"Cannot invoke brand performance: target_brand missing/invalid in {query_subjects}")
            aggregated_results["errors"].append({"source": "Router", "error": "Cannot invoke brand performance lambda",
                                                 "details": "Target brand domain not found"})

    # Amazon Radar
    if "internal_amazon_radar" in required_sources:
        target_department = original_context.get("target_department")
        target_category_for_amazon = original_context.get("target_category")
        if target_department and target_category_for_amazon:
            payload = {"queryStringParameters": {"country": country_name, "category": target_category_for_amazon,
                                                 "department": target_department, "sort_type": "revenue_descending"}}
            tasks_to_submit.append(("amazon", AMAZON_RADAR_LAMBDA_NAME, payload, None))
        else:
            logger.error(
                f"Cannot invoke Amazon Radar: target_department or target_category missing in {original_context}")
            aggregated_results["errors"].append({"source": "Router", "error": "Cannot invoke Amazon Radar lambda",
                                                 "details": "Target department or category not found"})

    # --- Execute tasks in parallel ---
    if tasks_to_submit:
        futures = []
        # Consider adjusting max_workers if many parallel calls are expected for comparisons
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            for task_id, lambda_name, payload, subject_name_for_task in tasks_to_submit:
                logger.info(f"Submitting task '{task_id}' for {lambda_name} with payload: {json.dumps(payload)}")
                futures.append(
                    executor.submit(invoke_lambda_task, lambda_name, payload, task_id, subject_name_for_task))
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    if error_info:
                        ...
                        logger.info(f"Task '{task_id}' completed successfully.")
                        # --- NEW: Handle comparison trend data ---
                        if task_id.startswith("trends_compare_"):
                            category_name_compared = task_result.get("category_compared")  # From invoke_lambda_task
                            if category_name_compared:
                                formatted_trend_data = {  # Structure consistent with single trend data
                                    "category_summary": data.get("country_category"),
                                    "style_details": data.get("country_category_style", []),
                                    "color_details": data.get("country_color_category", [])
                                }
                                aggregated_results["trends_data_comparison"].append({
                                    "category_name": category_name_compared,
                                    "data": formatted_trend_data
                                })
                                logger.info(f"Stored comparison trend data for category: {category_name_compared}")
                            else:
                                logger.error(
                                    f"Task '{task_id}' (comparison) missing category_compared field in result.")
                                aggregated_results["errors"].append(
                                    {"source": task_id, "error": "Comparison trend data missing category identifier"})
                        # --- END NEW ---
                        elif task_id == "trends_single":  # Standard single trend data
                            aggregated_results["trends_data"] = {"category_summary": data.get("country_category"),
                                                                 "style_details": data.get("country_category_style",
                                                                                           []),
                                                                 "color_details": data.get("country_color_category",
                                                                                           [])}
                        elif task_id == "mega":
                            aggregated_results["mega_trends_data"] = data.get("query_category", [])
                        elif task_id == "charts":
                            aggregated_results["chart_details_data"] = data
                        elif task_id == "brand_perf":
                            if isinstance(data.get("performance_data"), list):
                                aggregated_results["brand_performance_data"] = data["performance_data"]; logger.info(
                                    f"Stored brand performance data for {data.get('brand_domain')}")
                            else:
                                logger.error(f"Task 'brand_perf' returned unexpected data format: {data}");
                                aggregated_results["errors"].append(
                                    {"source": BRAND_INSIGHT_LAMBDA_NAME, "error": "Invalid data format received",
                                     "details": "Expected 'performance_data' list key."})
                        elif task_id == "amazon":
                            aggregated_results["amazon_radar_data"] = data
                            logger.info(
                                f"Stored Amazon Radar data. Keys: {list(data.keys()) if isinstance(data, dict) else 'Not a dict'}")
                    else:
                        logger.error(f"Task '{task_id}' returned no data and no error."); aggregated_results[
                            "errors"].append({"source": task_id, "error": "Task returned unexpected empty result"})
                except Exception as e:
                    logger.exception(f"Error retrieving result from future: {e}");
                    aggregated_results["errors"].append(
                        {"source": "ThreadPoolExecutor", "error": f"Failed to get task result: {str(e)}"})
    else:
        logger.info("No downstream Lambdas needed based on required_sources.")

    # --- Determine final status ---
    attempted_fetch = bool(tasks_to_submit)
    if not aggregated_results["errors"]:
        if attempted_fetch:
            aggregated_results["status"] = "success"; logger.info("Internal data fetch step completed successfully.")
        else:
            aggregated_results["status"] = "success_noop"; logger.info("No internal data fetch required.")
    else:
        logger.warning(
            f"Internal data fetch step completed with {len(aggregated_results['errors'])} error(s). Status: partial.")
    ...
    data_presence = {
        k: aggregated_results.get(k) is not None
        for k in ["trends_data", "mega_trends_data", "chart_details_data",
                  "brand_performance_data", "amazon_radar_data", "trends_data_comparison"]
    }
    # Log if comparison data is present
    if aggregated_results.get("trends_data_comparison"):
        logger.debug(f"Comparison data present with {len(aggregated_results['trends_data_comparison'])} items.")
